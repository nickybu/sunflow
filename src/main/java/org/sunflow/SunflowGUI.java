...
public class SunflowGUI extends javax.swing.JFrame implements UserInterface {
    ...
    public static void main(String[] args) {
        System.out.println("Rendering in Sunflow...");
        ...
        if (args.length > 0) {
            ...
            String brdfAlias = null;

            while (i < args.length) {
                // Custom args
                if (args[i].equals("-brdfAlias")) {
                    if (i > args.length - 2)
                        usage(false);
                    brdfAlias = args[i + 1];
                    i += 2;
            ...
            for (int frameNumber = frameStart; frameNumber <= frameStop; frameNumber++) {
                SunflowAPI api = SunflowAPI.create(input, frameNumber, brdfAlias);
                if (api == null)
                    continue;
                if (noRender)
                    continue;
    ...
    private void initGUI() {
        ...
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        {
            ...
            if (screenRes.getWidth() <= DEFAULT_WIDTH || screenRes.getHeight() <= DEFAULT_HEIGHT)
                ...
                desktop.setPreferredSize(new Dimension(DEFAULT_WIDTH, DEFAULT_HEIGHT));
            {
            ...
            }
            {
                ...
                editorFrame.setIconifiable(true);
                {
                    ...
                    jScrollPane1.setPreferredSize(new Dimension(360, 280));
                    {
