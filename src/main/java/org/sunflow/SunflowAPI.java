...
public class SunflowAPI implements SunflowAPIInterface {
    ...
    private BRDF currentCustomBRDF;
    ...
    public final void parameter(String name, BRDF brdf) {
        parameterList.addBRDF(name, brdf);
    }
     ...
     */
    public static SunflowAPI create(String filename, int frameNumber, String brdfAlias) {
        if (filename == null)
            return new SunflowAPI();
        SunflowAPI api = null;
        BRDF brdf = null;
        if(brdfAlias != null) {
            BRDFManager brdfManager = new BRDFManager();
            try {
                brdf = brdfManager.getBRDFFromAlias(brdfAlias);
                if(brdf == null) {
                    UI.printError(Module.API, "Could not find BRDF: \"%s\"", brdfAlias);
                }
            } catch (Exception e) {
                UI.printError(Module.API, "Could not initialise BRDF: \"%s\"", brdfAlias);
                e.printStackTrace();
            }
        }
    ...
    public BRDF getCurrentCustomBRDF() {
        return currentCustomBRDF;
    }

    public void setCurrentCustomBRDF(BRDF currentCustomBRDF) {
        this.currentCustomBRDF = currentCustomBRDF;
    }
