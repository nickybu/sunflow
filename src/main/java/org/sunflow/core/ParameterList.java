...
public class ParameterList {
    ...
    private enum ParameterType {
        STRING, INT, BOOL, FLOAT, POINT, VECTOR, TEXCOORD, MATRIX, COLOR, BRDF
    }
    ...
    public void addBRDF(String name, BRDF brdf) {
        add(name, new Parameter(brdf));
    }
    ...
    public BRDF getBRDF(String name, BRDF defaultValue) {
        Parameter p = list.get(name);
        if(isValidParameter(name, ParameterType.BRDF, InterpolationType.NONE, -1, p)) {
            return p.getBRDF();
        }
        return defaultValue;
    }
    ...
    private boolean isValidParameter(String name, ParameterType type, InterpolationType interp, int requestedSize, Parameter p) {
        ...
        if (requestedSize > 0 && Math.abs(p.size()) != requestedSize) {
    ...
    protected static final class Parameter {
        ...
        private Parameter(BRDF brdf) {
            type = ParameterType.BRDF;
